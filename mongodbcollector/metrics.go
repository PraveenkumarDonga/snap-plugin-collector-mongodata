...
//added this for extracting the data:
func getmongodata(status mwrapper.ServerStatus, mts []plugin.Metric, meta map[string]string) []plugin.Metric {
	metrics := []plugin.Metric{}
	...
	for _, mt := range mts {
		...
		switch ns[nsSubMetric].Value {
		case "data_1":
		     //	metrics = append(metrics, createMeasurement(mt, status.Network.BytesIn/*needs to be edited*/, ns, meta))
			metrics = append(metrics, createMeasurement(mt,Data, ns, meta))
		}
	}
	return metrics
}
